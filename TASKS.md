...
- **P1 D13 descoperit de Reviewer:** `doar din NP 010 2099` poate consuma aliasul scurt `NP010` și selecta greșit documentul 2022, în loc de `ambiguous_reference`; repro host în `multi-document/d13-whitespace-suffix-repro.json`. Acceptarea D11 este **BLOCATĂ**. QA a fost întrerupt de limita providerului; nu pretindem verdict QA.
- **Unicul task activ:** fix P1 D13 + regresii retrieval/API pentru an/partea numerică separată prin whitespace; apoi host focused/full, checkpoint. Fără task/agent paralel; fără merge/deploy, DB real sau API plătit.
