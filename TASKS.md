...
- **P1 exact-article remediat local:** cod complet + articol cunoscut nemarcat sau cu `art.` ajunge la `find_exact`, zero embedding; `NP 010 2099` rămâne fail-closed. Host: 105 focused passed, **962 full passed/11 skipped/1 warning**, exit 0; repro `p1-exact-article-fixed-repro.json`. Urmează checkpoint GitHub, apoi re-review P1 ca task separat. Nu există merge/deploy, DB real sau API plătit.
