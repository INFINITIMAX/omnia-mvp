...
- **Reviewer final: BLOCKED/P1.** Fixul P1 respinge articolul exact nemarcat după cod complet (`NP 010-2022 4.4.7.2`) înainte de exact lookup; controlul cu `art.` rămâne corect. Repro comparativ host: `multi-document/p1-exact-article-regression-repro.json`. Unicul task următor este regresia exact-article P1, cu negativele 2099 păstrate. Nu există merge/deploy, DB real sau API plătit. Respectăm regula: checkpoint, apoi decizie explicită de continuare.
