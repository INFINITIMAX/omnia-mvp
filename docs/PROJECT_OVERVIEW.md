...
- **Workflow manual local:** preflight-ul acceptat local `manual_ingestion_preflight.py` primește explicit un PDF din `_inbox` și scrie un raport local fără text normativ după SHA/extracție/chunking/metadata candidat. Host: 14 focused și 976 full passed/11 skipped; QA recheck și Reviewer P2 OK. DB + Voyage cer aprobare separată, iar publicarea (`approved`) încă una. Importerul DB sigur punctual nu este încă livrat; `populare_db.py` nu este insert-only.
