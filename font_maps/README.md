...
**Doua rafinari suplimentare ale proxy-ului:**
- *Nepotrivire de lungime, nu doar lipsa totala*: initial foloseam proxy-ul doar cand dict dadea 0 caractere. Am descoperit (caz real, verificat) ca dict poate da si un numar GRESIT de caractere fara sa lipseasca de tot — fie MAI PUTINE (un glif omis), fie MAI MULTE (un span vecin, apartinand altui candidat, s-a lipit de needle). Acum proxy-ul intervine ori de cate ori lungimea nu corespunde exact numarului de GID-uri, nu doar la lipsa totala.
- *Interpolare liniara verificata* (`_completeaza_proxy_prin_interpolare`): fallback-ul stricat al PyMuPDF atribuie codpoint-uri SECVENTIAL pe intervale mici de GID-uri (verificat: gid 882->U+0372 ... gid 891->U+037B, o progresie de pas 1 pe 10 GID-uri consecutive). Cand doua GID-uri invatate direct au intre ele un gol mic (≤8) SI diferenta lor de codpoint e EXACT egala cu diferenta de GID, completam GID-urile din mijloc prin aceeasi progresie — nu e o presupunere, e o extrapolare a unui tipar deja confirmat de doua puncte reale. O progresie care nu se potriveste exact NU e completata.
- Un singur GID (958 = radical √, folosit doar de doua ori, ambele in interiorul unei fractii 2D stivuite fara nicio potrivire de incredere de la care sa invete) a fost verificat manual prin doua cai independente convergente (interpolare cu vecinul gid 959, plus gasirea literala a caracterului asteptat exact la pozitia formulei in text) si adaugat explicit in `PROXY_VERIFICAT_MANUAL`, documentat in cod.

Cu aceste rafinari, aparitiile ramase nereparate din i9_2022 au scazut initial la aproximativ 92 de caractere raportate — cifra care insa **includea o eroare de masurare**: intervalul folosit pentru "caracter stricat" (U+0370-03FF, "Greek and Coptic") acoperea din greseala si literele grecesti NORMALE folosite legitim in formule (Σ, ψ, θ, ρ, λ, ζ, Φ, α, ν, φ, π, Δ etc.), nu doar codpoint-urile arhaice/nealocate pe care le foloseste fallback-ul stricat al PyMuPDF (ex. U+0372-037B). Corectat (exclus explicit litera-cu-litera intervalul Alpha-Omega/alpha-omega, vezi `GREEK_LITERE_LEGITIME` in `glyph_mapping.py`), reziduul real masurat a fost **36 de caractere**, din care **1 singur** caz s-a dovedit a fi un rest de corectie (nu o formula nerezolvata): un needle scurt "furat" de un candidat identic de pe aceeasi pagina lasa caracterul stricat original neconsumat, lipit de propria lui varianta deja reparata a ACELUIASI GID (vezi `_curata_duplicate_adiacente` mai jos).

Dupa curatarea acelui caz, reziduul final, verificat, este de **35 de caractere din cele 3093 CambriaMath din i9_2022 (~1,1%)** — plus, in afara scopului acestei probleme, ~20 de caractere "suspecte" care sunt de fapt typo-uri preexistente in alte fonturi (ex. "centralǎ" in loc de "centrală", in TimesNewRomanPSMT), nelegate de CambriaMath.

**Curatarea resturilor lipite de text deja reparat** (`_curata_duplicate_adiacente`): pentru un needle scurt care se potriveste identic la mai multi candidati de pe aceeasi pagina (text repetat — ex. aceeasi formula apare de doua ori), `aplica_inlocuiri()` cauta prima aparitie libera in tot textul, nu neaparat cea corespunzatoare candidatului respectiv (vezi limitarea de mai jos). Rezultatul poate fi un caracter stricat ramas neconsumat chiar langa versiunea deja corectata a ACELUIASI GID (verificat: art. 14.9, pag. 122 — "V" reparat, cu un "s" stricat ramas lipit intre V si punctul suprapus, inainte de indicele "_{s,zi,ac}"). Aplicam o curatare finala, CONSERVATOARE: stergem un caracter stricat doar cand (a) e o valoare cunoscuta din proxy (deci corespunde unui GID identificat cu certitudine), (b) caracterul corect al ACELUIASI GID exista deja alaturi — inainte sau dupa, sarind peste markup de indice/exponent (`_{ }`/`^{ }`), spatii si punctul suprapus combinat — si (c) nu e niciodata o litera greaca normala (verificare suplimentara de siguranta, independenta de (a)). Nu stergem niciodata doar dupa interval Unicode. Verificat pe tot documentul: din cele 36 de caractere stricate ramase inainte de aceasta curatare, regula a gasit si sters exact 1 potrivire certa.

Ce ramane, verificat, nu ascuns (35 de caractere):

- **Formule cu fractie/constructie 2D stivuita reala** (ex. art. 11.13 "[l/s]" randat ca fractie verticala l peste s; formulele f_AR/f_AC de la art. 11.6-11.7 cu radicalul intr-un numitor stivuit; ψ=1/(2π·λ·ln(...)) de la art. 11.16; ΣQ_j de la art. 11.15; numarul ecuatiei (11.18); h_d=i·l=λ/D·v²/2g de la art. dedicat pierderilor liniare; formula pompei H_RH de la pag. 53; V̇_recirculare=Q·4180·x·∆θ de la pag. 135): `page.get_text("dict")` pune bucatile pe "linii" diferite intr-o ordine care nu respecta citirea naturala (numarator/linie de fractie/numitor pe randuri separate, nu in ordinea "stanga-dreapta" pe care o presupune needle-ul nostru asamblat dupa x0). Corectarea IDENTITATII glifelor ramane corecta (toate componentele individuale sunt cunoscute), doar ORDINEA de asamblare a needle-ului nu (mai) corespunde textului real. Rezolvarea completa ar cere reconstruirea ordinii de citire a unui layout 2D, nu doar maparea de caractere — o extensie arhitecturala distincta, in afara scopului acestei sarcini. **Decizie**: se lasa asa cum e, documentat — o rescriere ar risca regresie pe restul, care functioneaza corect.
- **Coliziuni rare intre corectii identice fara ancorare pozitionala, cand NICIO aparitie libera reala nu mai exista** (varianta reziduala a limitarii de mai sus, dupa aplicarea curatarii): cand needle-ul "furat" apartine unei formule 2D deja esuate (deci nu exista de fapt un al treilea loc liber corect unde sa "aterizeze" corectia), curatarea de duplicate nu are ce sa gaseasca — ramane parte din reziduul de mai sus, nu o categorie separata de rezolvat.
