...
**APROBAT: direcția și reluarea locală. EXECUȚIE BLOCATĂ înainte de RED (11-09-2026).** Coderul a fost reluat explicit de Lucian și a salvat numai testele RED `tests/test_multi_document_retrieval.py`, fără runtime. Host RED D11–D13: 42 failed/35 passed/0 errors/0 skipped, exit 1 comportamental (`multi-document/red.receipt.json`): confirmă filtrarea automată după citări/context, ambiguitatea artificială la două documente, slash nerezolvat și D12/D13 neimplementate. D11 are 2/8 gate-uri locale închise: baseline + RED. Nu există `runtime-authorized.txt`, implementare, QA sau review; D12/D13 sunt acum aprobate pentru restricții: „doar/numai/exclusiv din [cod]”, numai pentru întrebarea curentă; cod necunoscut/neaprobat → `ambiguous_reference` existent, fără embedding/global/generare/apel plătit, cu quota/rate limit consumate. Negările, multiplele restricții, continuările ambigue și replay-ul UI rămân D02/D03 în afara acestui runtime. Testele RED D12/D13 se adaugă înainte de runtime. **D14 aprobat ulterior:** exact „nu doar din [cod]” rămâne global, fără scope; nu extindem la alte negații. Coderul a blocat corect runtime-ul înainte de editări până la această aprobare; se reia numai după checkpoint-ul deciziei. Inspecția/rerularea pentru [`HANDOFF.md`](HANDOFF.md) confirmă R06/Python pre-D11 intacte, 761 passed/11 skipped și diagnostic 19 cazuri/10 constatări R05/zero pasaje omise/zero erori. Nicio operațiune externă/publicare. Acoperă R01 și filtrarea/contextul semantic; nu declară R02–R04 complet rezolvate. Înlocuiește recomandările vechi care echivalau un răspuns multi-document cu o eroare.
...
| Cod necunoscut într-o restricție, referință de continuare ambiguă, persistența restricției | D01/D02: nu alegem tacit mesaj/status/quota ori durata restricției. Coder semnalează ce blochează contractul înainte să atingă ramura. |
